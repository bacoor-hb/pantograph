...
function getSelectedAsset (state) {
  ...
  return selectedToken && selectedToken.symbol || 'TOMO'
}
...
function getIsMainnet (state) {
  ...
  return networkType === NETWORK_TYPES.TOMO_MAINNET
}

function isTomoChainNetwork (state) {
  const networkType = getNetworkIdentifier(state)
  const {
    TOMO_MAINNET,
    TOMO_TESTNET
  } = NETWORK_TYPES
  return [TOMO_MAINNET, TOMO_TESTNET].includes(networkType)
}
