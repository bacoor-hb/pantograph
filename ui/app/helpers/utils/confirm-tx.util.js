...
export function formatCurrency (value, currencyCode, decimals = 6) {
  const upperCaseCurrencyCode = currencyCode.toUpperCase()
  const formatObject = { code: upperCaseCurrencyCode, style: 'currency',  }
  if (decimals) {
    formatObject.precision = decimals
  }
  return currencies.find(currency => currency.code === upperCaseCurrencyCode)
    ? currencyFormatter.format(Number(value), formatObject)
    : value
}
...
export function convertSmallNumberToRegular (x) {
  if (Math.abs(x) < 1.0) {
