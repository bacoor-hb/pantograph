...
const { updateSendErrors } = require('../ducks/send/send.duck')
  ...
  return (dispatch) => {
    ...
    return estimateGas({
      ...
      .then(gas => {
        ...
        dispatch(updateSendErrors({ estimateGas: null }))
        dispatch(actions.gasLoadingFinished())
      })
      .catch(err => {
        log.error(err)
        dispatch(updateSendErrors({ estimateGas: 'estimateGasError' }))
        dispatch(actions.gasLoadingFinished())
      })
  }
}
...
function setTokenIssuer (tokenIssuer) {
  return {
