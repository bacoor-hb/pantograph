...
export default class ConfirmTransactionBase extends Component {
  ...
  getErrorKey () {
    ...
    const insufficientBalance = balance && !isBalanceSufficient({
      amount,
      gasTotal: tokenIssuer && tokenIssuer.feeFund ? '0x0' : hexTransactionFee,
      balance,
      conversionRate,
    })
        ...
        <div className="confirm-page-container-content__details">
          ...
          <div className={useNonceField ? 'confirm-page-container-content__gas-fee' : null}>
            <ConfirmDetailRow
              ...
              secondaryText={hideFiatConversion ? this.context.t('noConversionRateAvailable') : ''}
              headerText={!tokenIssuer || tokenIssuer && !tokenIssuer.feeFund ? "Amount + Gas Fee" : "Amount"}
  ...
  renderSubtitleComponent () {
    const { subtitle, subtitleComponent, hexTransactionAmount, tokenIssuer, selectedToken } = this.props

    // Subtitle string passed in by props takes priority
    if (subtitle) {
      return null
    }
    if (selectedToken && tokenIssuer && tokenIssuer.feeFund && tokenIssuer.feeFund > 0) {
      return null
    }
  ...
  render () {
    ...
    return (
      <ConfirmPageContainer
        ...
        action={getMethodName(name) || this.context.tOrKey(transactionCategory === 'sentEther' ? 'sendETH' : transactionCategory) || this.context.t('contractInteraction')}
        title={title}
