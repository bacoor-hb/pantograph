...
function isTokenIssuerBalanceSufficient ({ 
  balance = '0x0',
  conversionRate = 1,
  gasTotal = '0x0',
  primaryCurrency 
...
function getAmountErrorObject ({
  ...
  const amountLessThanZero = conversionGTE(
...
function getGasFeeErrorObject ({
  ...
  if (gasTotal && conversionRate) {
    const insufficientFunds = !isBalanceSufficient({
      ...
      balance,
      conversionRate,
      gasTotal: tokenIssuer && tokenIssuer.feeFund ? '0x0' : gasTotal,
      primaryCurrency,
    })
...
function fetchTokenIssuer (selectedTokenAddress, network) {
  const minABI = [
    {
      "constant":true,
      "inputs":[
  ...
  const contract = global.eth.contract(minABI).at(Number(network) === TOMO_MAINNET_CODE ? '0x8c0faeb5c6bed2129b8674f262fd45c4e9468bee' : '0x0e2c88753131ce01c7551b726b28bfd04e44003f')
  return new Promise((resolve, reject) => {
    return contract.getTokenCapacity(selectedTokenAddress, (err, res) => {
      if (err) {
        return reject(false)
      }
      let balance = new BigNumber(res[0].toString())
      balance = balance.div(10 ** 18).toNumber()
      return resolve({
        issuerAddress: Number(network) === TOMO_MAINNET_CODE ? '0x8c0faeb5c6bed2129b8674f262fd45c4e9468bee' : '0x0e2c88753131ce01c7551b726b28bfd04e44003f',
        feeFund: balance
      })
    })
  })
