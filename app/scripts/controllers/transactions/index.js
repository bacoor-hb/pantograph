    ...
    return new Promise((resolve, reject) => {
      this.txStateManager.once(`${initialTxMeta.id}:finished`, (finishedTxMeta) => {
        switch (finishedTxMeta.status) {
          ...
          case 'rejected':
            return reject(cleanErrorStack(rpcErrors.eth.userRejectedRequest('Pantograph Tx Signature: User denied transaction signature.')))
          case 'failed':
            return reject(cleanErrorStack(rpcErrors.internal(finishedTxMeta.err.message)))
          default:
            return reject(cleanErrorStack(rpcErrors.internal(`Pantograph Tx Signature: Unknown problem: ${JSON.stringify(finishedTxMeta.txParams)}`)))
        }
      })
    })
  }
