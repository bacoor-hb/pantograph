...
const networkTypeToIdMap = {
  ...
  [TOMO_MAINNET]: String(TOMO_MAINNET_CODE),
  [TOMO_TESTNET]: String(TOMO_TESTNET_CODE),
}
...
class IncomingTransactionsController {
  ...
  async _fetchTxs (address, fromBlock, networkType) {
    ...
    let tomoscanSubdomain = 'scan'
    let apiUrl = ''
    const currentNetworkID = networkTypeToIdMap[networkType]
    const supportedNetworkTypes = [ROPSTEN, RINKEBY, KOVAN, GOERLI, MAINNET, TOMO_MAINNET, TOMO_TESTNET]
    const isTomoChainNetwork = [TOMO_MAINNET, TOMO_TESTNET].includes(networkType)

    if (supportedNetworkTypes.indexOf(networkType) === -1) {
      return {}
    }

    if (isTomoChainNetwork) {
      if (networkType !== TOMO_MAINNET) {
        tomoscanSubdomain = `scan.testnet`
      }
      apiUrl = `https://${tomoscanSubdomain}.tomochain.com/api/txs`
      apiUrl += `?txs?address=${address}`
    } else {
      if (networkType !== MAINNET) {
        etherscanSubdomain = `api-${networkType}`
      }
      apiUrl = `https://${etherscanSubdomain}.etherscan.io/api`
      apiUrl += `?module=account&action=txlist&address=${address}&tag=latest&page=1`
  
      if (fromBlock) {
        apiUrl += `&startBlock=${parseInt(fromBlock, 10)}`
      }
    } 
  ...
  _processTxFetchResponse ({ items = [], address, currentNetworkID }) {
  ...
  _normalizeTxFromEtherscan (txMeta, currentNetworkID) {
    ...
    const status = txMeta.status === true ? 'confirmed' : 'failed'
    return {
