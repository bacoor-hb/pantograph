...
module.exports = class PersonalMessageManager extends EventEmitter {
  ...
  addUnapprovedMessageAsync (msgParams, req) {
    return new Promise((resolve, reject) => {
      if (!msgParams.from) {
        reject(new Error('Pantograph Message Signature: from field is required.'))
      }
      ...
      this.once(`${msgId}:finished`, (data) => {
        switch (data.status) {
          ...
          case 'rejected':
            return reject(rpcErrors.eth.userRejectedRequest('Pantograph Message Signature: User denied message signature.'))
          default:
            return reject(new Error(`Pantograph Message Signature: Unknown problem: ${JSON.stringify(msgParams)}`))
        }
      })
    })
  }
