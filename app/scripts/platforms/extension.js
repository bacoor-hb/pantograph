...
const prefixForNetwork = require('../../../ui/lib/tomochain-prefix-for-network')
...
class ExtensionPlatform {
  ...
  _showConfirmedTransaction (txMeta) {
    ...
    const prefix = prefixForNetwork(txMeta.metamaskNetworkId)
    const url = `https://scan.${prefix}tomochain.com/txs/${txMeta.hash}`
    const nonce = parseInt(txMeta.txParams.nonce, 16)

    const title = 'Confirmed transaction'
    const message = `Transaction ${nonce} confirmed! View on TomoScan`
