...
const TomoKeyring = require('./tomo-keyring')
...
module.exports = class MetamaskController extends EventEmitter {
  ...
  constructor (opts) {
    ...
    // key mgmt
    const additionalKeyrings = [TrezorKeyring, LedgerBridgeKeyring, TomoKeyring]
  ...
  getApi () {
    ...
    return {
      ...
      updateToken: nodeify(preferencesController.updateToken, preferencesController),
   ...
   */
  async createNewVaultAndKeychainOld (password) {
  ...
  async createNewVaultAndKeychain (password) {
    ...
    try {
      ...
      if (accounts.length > 0) {
        ...
        vault = await this.createNewVaultAndKeychainTomoChain(password)
        const accounts = await this.keyringController.getAccounts()
        this.preferencesController.setAddresses(accounts)
        this.selectFirstIdentity()
      }
  ...
  async createNewVaultAndKeychainTomoChain (password) {
    const keyringController = this.keyringController
    ...
    return keyringController.persistAllKeyrings(password)
      .then(() => {
        return keyringController.addNewKeyring('Tomo Key Pair', {
          numberOfAccounts: 1,
        })
      })
      .then((firstKeyring) => {
      ...
      .then((accounts) => {
        ...
        this.keyringController.emit('newVault', hexAccount)
        return null
      })
      .then(keyringController.persistAllKeyrings(password))
      .then(keyringController.fullUpdate())
  }

  async createFirstKeyTreeTomo () {
    this.keyringController.clearKeyrings()
    return this.keyringController.addNewKeyring('Tomo Key Pair', { numberOfAccounts: 1 })
      .then((keyring) => {
   ...
   * @param  {} seed
   */
  async createNewVaultAndRestoreOld (password, seed) {
    const releaseLock = await this.createVaultMutex.acquire()
    try {
      ...
      if (!primaryKeyring) {
        throw new Error('PantographController - No HD Key Tree found')
      }
  ...
  /**
   * Create a new Vault and restore an existent keyring in TomoChain
   * @param  {} password
   * @param  {} seed
   */
  async createNewVaultAndRestore (password, seed) {
    ...
    try {
      ...
      this.preferencesController.setAddresses([])
      // create new vault
      const vault = await this.createNewVaultAndRestoreTomoAccount(password, seed)

      const ethQuery = new EthQuery(this.provider)
      accounts = await keyringController.getAccounts()
      lastBalance = await this.getBalance(accounts[accounts.length - 1], ethQuery)

      const primaryKeyring = keyringController.getKeyringsByType('Tomo Key Pair')[0]
      if (!primaryKeyring) {
        throw new Error('MetamaskController - No Tomo Key Pair found')
      }
  ...
  async createNewVaultAndRestoreTomoAccount (password, seed) {
  ...
  async fetchInfoToSync () {
    ...
    Object.keys(accountTokens).forEach(address => {
      ...
      Object.keys(accountTokens[address]).forEach(
        networkType => (filteredAccountTokens[checksummedAddress][networkType] = networkType !== 'tomo_mainnet' ?
    ...
    // Accounts
    const hdKeyring = this.keyringController.getKeyringsByType('Tomo Key Pair')[0]
  ...
  async submitPassword (password) {
    ...
    // verify keyrings
    const nonSimpleKeyrings = this.keyringController.keyrings.filter(keyring => keyring.type !== 'Tomo Key Pair')
  ...
  async getKeyringForDevice (deviceName, hdPath = null) {
    ...
    switch (deviceName) {
      ...
      case 'ledger':
        ...
        break
      default:
        throw new Error('PantographController:getKeyringForDevice - Unknown device')
    }
   ...
   */
  async addNewAccount () {
    const primaryKeyring = this.keyringController.getKeyringsByType('Tomo Key Pair')[0]
    if (!primaryKeyring) {
      throw new Error('PantographController - No Tomo Key Pair found')
    }
    const keyringController = this.keyringController
  ...
  async verifySeedPhrase () {
    ...
    if (accounts.length < 1) {
      throw new Error('PantographController - No accounts found')
    }
    try {
  ...
  signMessage (msgParams) {
    log.info('PantographController - signMessage')
  ...
  signPersonalMessage (msgParams) {
    log.info('PantographController - signPersonalMessage')
    const msgId = msgParams.metamaskId
    // sets the status op the message to 'approved'
    // and removes the metamaskId for signing
  ...
  async signTypedMessage (msgParams) {
    log.info('PantographController - eth_signTypedData')
    const msgId = msgParams.metamaskId
    ...
    try {
      ...
      log.info('PantographController - eth_signTypedData failed.', error)
  ...
  setupUntrustedCommunication (connectionStream, senderUrl, extensionId) {
    ...
    if (this.phishingController.test(senderUrl.hostname)) {
      log.debug('Pantograph - sending phishing warning for', senderUrl.hostname)
  ...
  async updateAndSetCustomRpc (rpcUrl, chainId, ticker = 'TOMO', nickname, rpcPrefs) {
   ...
   * @returns {Promise<String>} - The RPC Target URL confirmed.
   */
  async setCustomRpc (rpcTarget, chainId, ticker = 'TOMO', nickname = '', rpcPrefs = {}) {
