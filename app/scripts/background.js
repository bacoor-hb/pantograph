...
async function initialize () {
  ...
  log.debug('Pantograph initialization complete.')
}
...
async function loadStateFromPersistence () {
  ...
  if (!versionedData) {
    throw new Error('Pantograph - migrator returned undefined')
  }
  ...
  if (localStore.isSupported) {
    ...
    setTimeout(() => {
      throw new Error('Pantograph - Localstore not supported')
    })
  }
...
function setupController (initState, initLangCode) {
  ...
  async function persistData (state) {
    if (!state) {
      throw new Error('Pantograph - updated state is missing', state)
    }
    if (!state.data) {
      throw new Error('Pantograph - updated state does not have data', state)
    }
    if (localStore.isSupported) {
      try {
